...
from ...openarm_lift_style_scene_cfg import (
    CUBE_NAMES,
    LIFT_STYLE_CUBE_SPAWN_POSITIONS as TABLETOP_CUBE_SPAWN_LOCAL_POSITIONS,
    LIFT_STYLE_STACK_BASE_LOCAL_POS as TABLETOP_STACK_BASE_LOCAL_POS,
)
...
def reset_cubes_tabletop(
    ...
    noise_xy = (
        ...
        else torch.zeros((len(env_ids), len(cube_names), 2), device=env.device)
    )
...
def reset_stack_target_tabletop(
    ...
    local_pos = torch.tensor(local_stack_base[:3], dtype=torch.float32, device=env.device)
    local_pos_batch = local_pos.unsqueeze(0).expand(len(env_ids), -1).clone()
    if position_noise > 0.0:
        local_pos_batch[:, :2] += torch.empty((len(env_ids), 2), device=env.device).uniform_(
            -position_noise, position_noise
        )

    env.stack_base_pos_w[env_ids] = env.scene.env_origins[env_ids] + local_pos_batch
