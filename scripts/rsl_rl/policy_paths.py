"""Resolve and deploy LL policy checkpoints via ``best_policy/``.

On play / export / evaluation, the latest (or requested) RSL-RL checkpoint under
``logs/`` is copied into ``best_policy/best_policy.pt`` (a fixed, stable name).
Downstream code (HL classical play, deterministic evaluation, real-robot
deployment) always loads from there, so it does not depend on timestamped log
run folders. Exported TorchScript / ONNX land under ``best_policy/exported/``.

All paths are resolved relative to the repository root, so nothing is hardcoded
...
def sync_best_policy(
    ...
    *,
    ...
    """Copy the chosen LL checkpoint into ``best_policy/best_policy.pt``.

    Resolution order:
      1. ``explicit_checkpoint`` when provided (``--checkpoint``).
      2. Latest match under ``logs/rsl_rl/<experiment_name>/``.
      3. Existing ``best_policy/best_policy.pt`` if no logs checkpoint is found.

    Returns the path to ``best_policy/best_policy.pt``.
    ...
    raise FileNotFoundError(
        ...
        f"{BEST_POLICY_CHECKPOINT}. Expected training logs under: {log_root_path(experiment_name)}"
    )
