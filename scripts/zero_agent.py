"""Run the OpenArm cube stacking environment with zero actions."""
...
parser = argparse.ArgumentParser(description="Run zero actions in the OpenArm cube stacking task.")
...
def main() -> None:
    ...
    zero_actions = torch.zeros((env.unwrapped.num_envs, *single_action_shape), device=env.unwrapped.device)

    with torch.inference_mode():
        for step in range(args_cli.steps):
            if not simulation_app.is_running():
                break
            _, rewards, terminated, truncated, _ = env.step(zero_actions)
            if step % 100 == 0:
